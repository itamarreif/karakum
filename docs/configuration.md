...
| `agents/<name>.yaml` | An agent identity: `name`, `memory.path` (local memory repo), `memory.repository` (canonical remote; preflight verifies the local `origin` matches), and optional `memory.init` (a shell command run in-container after mounts land, from `$KARAKUM_MEMORY` — karakum runs it verbatim and stays framework-agnostic; e.g. `ln -sfn "$KARAKUM_MEMORY/MASTER_PROMPT.md" "$HOME/.claude/CLAUDE.md"` to load a vault's master prompt. `$KARAKUM_TOOLCHAIN` is available; idempotent, non-fatal). Loaded by `manifest.load`. |
