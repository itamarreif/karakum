...
just shell alice web,api fix-login   # several projects — comma-separated, all mounted RW
...
just shell <agent> <project[,project...]> <slug>     # session with one or more projects mounted RW
...
`<slug>` names what the session is about. The launcher creates (or reuses) an **isolated clone** at `<sessions_root>/<agent>/<slug>/<label>` for **both** the memory repo (`label` = `scratchpad`) and the project repo (`label` = the project name), if specified. The clones sit together under `<agent>/<slug>` but check out **differently namespaced branches**: every project clone is on `<agent>/<slug>` (whose changes are the agent's), while the memory clone is on `<project>/<slug>` — so a memory repo shared across projects keeps each project's session work on its own branch. A session spanning several projects joins them with `+`, sorted, so `dewey,mundaneum` and `mundaneum,dewey` are the same `dewey+mundaneum/<slug>` branch rather than two. (With no project, the memory branch is just `<slug>`.) Grouping by session keeps every repo a session touches together, and living outside the repos means it never collides with a manual `git worktree add`. Each clone is fully independent (its own `.git`, no shared objects), so the agent can never touch the host repo's git database; its `origin` points at GitHub, so commits reach the host via push + pull/PR, not a shared `.git`. The slug is the stable session identity — resuming the same slug (even on a later day) reuses the same clone and branch.

**Reuse never switches branches.** An existing clone is left on whatever branch it is on, so relaunching a slug with a *different* set of projects reuses the clones and leaves the memory clone on the branch the first launch derived (`dewey+mundaneum/<slug>`, not the `dewey/<slug>` the second launch asked for). The launcher warns and names both; check out the other branch yourself if that is what you meant.

**Two projects whose repositories share a basename can't be mounted together** — both would land on `~/<name>` and share one clone. The launcher refuses, before creating anything. Launch them in separate sessions. Omitting the slug (`-`) skips cloning and mounts the live main branch directly — a warning is printed since changes affect the repo immediately.
...
To **reopen** an existing session without retyping its agent and project, use `just resume <slug>`: it finds the session's clones on disk, recovers the agent and **every** project from them, and drops you back into the same branches. If the slug exists under more than one agent it lists them and errors — qualify it as `just resume <agent>/<slug>`, or fall back to the explicit `just shell <agent> <project> <slug>` (which is also how you reopen a *subset* of a multi-project session). `just shell …` stays the way to **create** a session.
...
The branch column folds in dirty (`*`) and unpushed (`↑N`) state; note a session's clones carry differently namespaced branches (memory on `<project>/<slug>`, every project on `<agent>/<slug>`). The pr-state column queries GitHub with `gh api repos/{owner}/{repo}/pulls` (`#N` for an open PR, else `merged`/`closed`, `no-pr` if none). It authenticates with the `GH_TOKEN` resolved from `secrets.yaml` — the same source `just shell` uses — so it works on the host even when your interactive `gh` is a shell-function wrapper; an already-exported `GH_TOKEN` is reused as-is. Without `gh` on PATH, or when the token is missing/rejected, the cell shows `?` and a stderr line names the repo + reason.
...
- **Project session clones** — one per project named, at host `<sessions_root>/<agent>/<slug>/<project>/`, each mounted **RW** at `~/<project>` (the repo name).
- **CWD** inside the container = `~` (home); the memory clone and every project sit as siblings under it.
...
- **Env vars**: `KARAKUM_MEMORY` (`~/<agent>`), `KARAKUM_PROJECT` (`~/<project>` — the **first** project, when set), `KARAKUM_PROJECTS` (all of them, colon-separated like `PATH`), `KARAKUM_SESSION`, `KARAKUM_AGENT`.
